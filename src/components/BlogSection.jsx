    ...
    <section className={`py-20 relative transition-colors duration-300 ${'bg-gray-900'}`}>
      <div className="max-w-6xl mx-auto px-4">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <div className="inline-block backdrop-blur border rounded px-4 py-2 mb-4 font-mono bg-slate-900/80 border-cyan-400/30">
            <span className={'text-green-400'}>$</span> <span className={'text-cyan-300'}>npm run blog</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400 font-mono">
            Latest Blog Posts
          </h2>
          <p className={`mt-4 font-mono ${'text-gray-400'}`}>
            <span className={'text-purple-400'}># </span>Thoughts on tech, development, and innovation
          </p>
        </motion.div>
          ...
          {blogPosts.map((post, i) => {
            ...
            return (
              <motion.a
                key={i}
                href="#"
                className="group rounded-lg p-6 transition-all cursor-pointer backdrop-blur border bg-slate-900/50 border-purple-400/20 hover:border-cyan-400/60"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <Icon className="text-cyan-400 group-hover:text-purple-400 transition-colors" size={32} />
                  <span className="text-xs px-3 py-1 rounded-full font-mono bg-cyan-400/20 text-cyan-300">
                    {post.category}
                  </span>
                </div>
        ...
        >
          <motion.a
            href="#"
            className="inline-block px-8 py-3 font-mono rounded hover:bg-cyan-500/30 transition-all border bg-cyan-500/20 border-cyan-400 text-cyan-300"
