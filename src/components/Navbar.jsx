  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navItems.map((item, i) => (
              <motion.a
                key={i}
                href={item.href}
                className="text-sm font-medium transition-colors relative group text-gray-300 hover:text-blue-400"
                whileHover={{ scale: 1.05 }}
              >
                {item.label}
        ...
        >
          <div className="px-2 pt-2 pb-3 space-y-1 backdrop-blur-md border-t bg-gray-800/95 border-gray-700">
            {navItems.map((item, i) => (
              <a
                key={i}
                href={item.href}
                onClick={() => setIsOpen(false)}
                className="block w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors text-gray-300 hover:bg-gray-700 hover:text-blue-400"
              >
                {item.label}
              </a>
            ))}
            <a
              href="#contact"
              className="block px-3 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-lg text-sm text-center"
              onClick={() => setIsOpen(false)}
            >
              Hire Me
            </a>
